...
def write_reports(rows: list[dict[str, object]], root: Path) -> tuple[Path, Path]:
    ...
    strong_path = root / "reports" / "strong_ml_1s_3s" / "summary.csv"
    ...
    if strong_path.is_file():
        ...
        if required.issubset(strong.columns):
            strong = strong.loc[strong["region"].eq("all")]
            strong_lines = [
            ...
            for row in strong.sort_values(["window_seconds", "decoder"]).itertuples(index=False):
                strong_lines.append(
                    f"| {int(row.window_seconds)}s | {row.variant} | {row.decoder} | "
                    f"{float(row.macro_f1_mean):.4f} ± {float(row.macro_f1_std):.4f} | "
                    f"{float(row.accuracy_mean):.4f} | {float(row.balanced_accuracy_mean):.4f} |"
                )
            strong_lines.extend(
                [
                    "",
